...
void LiquidCrystal::init(uint8_t fourbitmode, uint8_t rs, uint8_t rw, uint8_t enable,
			 ...
			 uint8_t d4, uint8_t d5, uint8_t d6, uint8_t d7)
{
	_rs_pin = rs;					// register select (RS) pin
  _rw_pin = rw;					// read/write (RW) pin
  _enable_pin = enable;	// start data read/write (E) pin
  _usingSpi = false;		// added [REA]
